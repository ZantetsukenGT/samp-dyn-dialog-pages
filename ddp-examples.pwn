...
/* Callback */
hook test_response(playerid, response, additional_data, listitem, string: inputtext[])
{
	if (!response)
		return 1;
	SendClientMessage(playerid, -1, "[DDP] You selected listitem ID: {666666}%i{FFFFFF}, extra: {666666}%i{FFFFFF}, inputtext: {666666}%s", listitem, additional_data, inputtext);
	return 1;
}
...
YCMD: test2(playerid, params[], help) // DIALOG_STYLE_TABLIST
{
	...
	DynDialog_Show(playerid, using public test_response<iiiis>, DIALOG_STYLE_TABLIST, "{FFFFFF}Test Dialog Name {FF00FF}DDP_Test", "Button 1", "Button 2", 27);
	return 1;
}
...
YCMD: test3(playerid, params[], help) // DIALOG_STYLE_TABLIST_HEADERS
{
	...
	DynDialog_Show(playerid, using public test_response<iiiis>, DIALOG_STYLE_TABLIST_HEADERS, "{FFFFFF}Test Dialog Name {FF00FF}DDP_Test", "Button 1", "Button 2", 32);
	return 1;
}